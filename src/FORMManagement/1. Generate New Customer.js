 ...
 * @param {Object} new_fields_of_interests - Object containing information about fields of interest.
...
function generateIDAndURLAdressForNewCustomer(eventData, fieldsOfInterests, formTemplatesData) {
  ...
  //Change Shape of the fields of interests 
  let new_fields_of_interests = {};

  for (const key in fieldsOfInterests) {
    const interestData = fieldsOfInterests[key];
    new_fields_of_interests[interestData.realName || key] = { ...interestData };
    delete new_fields_of_interests[interestData.realName || key].realName;
  }
  ...
  for (let i = 0; i < arrayOfInterests.length; i++) {
    ...
    if (new_fields_of_interests.hasOwnProperty(interest)) {
      const interestData = new_fields_of_interests[interest];
