...
function folderToSetData(mainFolderID, folderName) {

    let foldersInMainFolder = listRootFolders_(mainFolderID)
    let folderData = foldersInMainFolder.items.find(folder =>
        (folder.name.toLowerCase() === folderName && folder.mimeType === 'application/vnd.google-apps.folder'))
